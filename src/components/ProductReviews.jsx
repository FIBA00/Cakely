  ...
  return (
    <section className="product-reviews" aria-labelledby="reviews-heading">
      <div className="reviews-heading">
        <div>
          <p className="eyebrow">{t("product.reviewEyebrow")}</p>
          <h2 id="reviews-heading">{t("product.reviewTitle")}</h2>
        </div>
        <span className="verified-review-note">
          <Star size={14} aria-hidden="true" /> {t("product.verifiedOnly")}
        </span>
      </div>
      {isLoading ? (
        ...
        <div className="reviews-empty">
          ...
          <div>
            <h3>{t("product.noReviews")}</h3>
            <p>{t("product.noReviewsCopy")}</p>
          </div>
        </div>
      )}
    </section>
