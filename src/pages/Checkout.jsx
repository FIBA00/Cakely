...
const getCheckoutSchema = t =>
  z
    .object({
      ...
      payment: z.enum(["Cash on Delivery", "Manual Bank Transfer"]),
      confirm: z
        .boolean()
        .refine(value => value, { message: t("validation.confirm") }),
    })
    .superRefine((values, context) => {
      ...
      if (values.fulfillment === "Delivery" && !values.city?.trim())
        context.addIssue({
          code: "custom",
          path: ["city"],
...
export default function Checkout() {
  ...
  function showPromoPreview(event) {
    event.preventDefault();
    setPromoMessage(t("checkout.promoPreview"));
  }
  async function onSubmit(data) {
    setSubmitError("");
    if (!online) {
      setSubmitError(t("checkout.offline"));
      return;
    }
    try {
      const order = await createOrder({
        customer: { name: data.name, phone: data.phone, email: data.email },
        fulfilment: data.fulfillment,
        address:
      ...
      clearCart();
      toast.success(t("status.confirmedTitle"));
  ...
  return (
    <section className="checkout-page page-width">
      ...
      <div className="checkout-preview-note" role="note">
        <strong>{t("checkout.previewMode")}</strong>
        <p>{t("checkout.previewCopy")}</p>
        <Link to="/policies">{t("checkout.policyLink")}</Link>
      </div>
      <section
        className="checkout-extras"
        aria-labelledby="checkout-extras-title"
      >
        <div>
          <h2 id="checkout-extras-title">{t("checkout.extras")}</h2>
          <p>{t("checkout.reminderPreview")}</p>
        </div>
        <form className="promo-form" onSubmit={showPromoPreview}>
          <label className="field">
            <span className="form-label">{t("checkout.coupon")}</span>
            <input
      ...
      <form
        ...
        aria-describedby="checkout-data-sharing"
      ...
      >
        <div className="checkout-form">
          ...
          >
            <div className="two-field">
              <Field label={t("checkout.name")} error={errors.name?.message}>
                <input
                  autoComplete="name"
                  className="form-input"
                  {...register("name")}
          ...
          >
            <div className="fulfilment-options">
              ...
              >
                ...
                <span>
                  ...
                  <small>{t("checkout.deliveryCopy")}</small>
                </span>
              </label>
              <label
                className={
              ...
              >
                ...
                <span>
                  ...
                  <small>{t("checkout.pickupCopy")}</small>
                </span>
              </label>
            </div>
            <p className="checkout-integration-note" role="note">
              {t("checkout.deliveryZonePreview")}
            </p>
            {fulfillment === "Delivery" && (
              <div className="two-field mt-5">
                <Field
            ...
            >
              <textarea
                ...
                placeholder={t("checkout.noteHint")}
          ...
          <CheckoutSection
            number="03"
            icon={<Check />}
            title={t("checkout.moment")}
          >
            <div className="two-field">
              <Field label={t("checkout.date")} error={errors.date?.message}>
                <input
                  className="form-input"
                  type="date"
                  min={minimumDate}
              ...
              <Field label={t("checkout.time")} error={errors.time?.message}>
                <select className="form-select" {...register("time")}>
            ...
            <div className="payment-options">
              ...
              <label>
                ...
                <span>
                  ...
                  <small>{t("checkout.bankCopy")}</small>
                </span>
              </label>
            </div>
          </CheckoutSection>
        ...
        <aside className="checkout-review">
          <p className="eyebrow">{t("checkout.review")}</p>
          {items.map(item => (
            <div className="review-item" key={item.lineId}>
          ...
          <div className="summary-row">
            ...
            <strong>
              {delivery ? formatPrice(delivery) : t("checkout.included")}
            </strong>
          </div>
          ...
          <label className="confirm-order">
            <input type="checkbox" {...register("confirm")} />
            <span>{t("checkout.check")}</span>
          </label>
          {errors.confirm && (
          ...
          <button
            ...
            className="btn-berry w-full mt-5"
            type="submit"
          >
          ...
          <p
            id="checkout-data-sharing"
            className="text-center text-xs leading-5 text-[#977b6e]"
          >
            {t("checkout.dataShared")}{" "}
            <Link className="text-link" to="/policies">
              {t("checkout.policyLink")}
            </Link>
          </p>
        </aside>
      </form>
...
function CheckoutSection({ number, icon, title, children }) {
  return (
