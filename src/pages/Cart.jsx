      ...
      <div className="cart-layout">
          ...
          {items.map(item => (
            <article className="cart-item" key={item.lineId}>
              ...
              <div className="cart-item-copy">
                <p className="eyebrow">{t("cart.made")}</p>
                <h2>{item.name}</h2>
        ...
        <aside className="order-summary">
          ...
          <div className="summary-row">
            ...
            <strong>
              {delivery ? formatPrice(delivery) : t("cart.included")}
            </strong>
          </div>
          <p className="delivery-hint">
            {subtotal < 90
              ? `${formatPrice(90 - subtotal)} away from included delivery.`
              : t("cart.included")}
          </p>
          ...
          <Link className="btn-berry w-full" to="/checkout">
            {t("cart.checkout")} <ArrowRight size={17} />
          </Link>
          <p className="summary-safe">
            <ShoppingBag size={15} /> {t("cart.safe")}
          </p>
        </aside>
      </div>
    </section>
