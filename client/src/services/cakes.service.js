...
export async function getCakes(params = {}) {
  const { query = "", category = "all", availability = "all", sort = "featured", price = "all", dietary = "all" } = params;
  let result = [...cakes];
  const term = query.trim().toLowerCase();
  if (term) result = result.filter((cake) => `${cake.name} ${cake.category} ${cake.flavor}`.toLowerCase().includes(term));
  if (category !== "all") result = result.filter((cake) => cake.category === category);
  if (availability === "available") result = result.filter((cake) => cake.available);
  if (dietary !== "all") result = result.filter((cake) => cake.dietary?.includes(dietary));
  if (price === "under-60") result = result.filter((cake) => cake.price < 60);
  if (price === "60-80") result = result.filter((cake) => cake.price >= 60 && cake.price <= 80);
